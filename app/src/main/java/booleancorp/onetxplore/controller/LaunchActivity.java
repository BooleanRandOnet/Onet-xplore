...
public class LaunchActivity extends Activity implements View.OnTouchListener{
    ...
    public void clicChangeStatu(View view) {
        createContent((int)actionBT.getTag());
        //enlever le fait que l'utilisateur s'est trompé lors de la co ()si il s'est trompé
        etPseudo.setBackgroundResource(R.drawable.inputshape);
        etMdp.setBackgroundResource(R.drawable.inputshape);
    }
                ...
                if (etPseudo.getText().length() > 0 && etPseudo.getText().length() > 0) {
                    ...
                    if ((int) sender.getTag() == 0) {
                        ...
                        if(!connexion) {
                            ...
                            final LaunchActivity that = this;
                            new Handler().postDelayed(new Runnable() {
                                @Override
                                public void run() {
                                    Intent mapIntent = new Intent(that, MainActivity.class);
